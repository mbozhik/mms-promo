...
def generate_promo_code(length=9):
...
# Функция для проверки кодов в отдельном потоке
def check_codes_thread():
    delay = 10  # Задержка между запросами (10 секунд)
    while True:
...
if __name__ == "__main__":
    ...
    for _ in range(num_threads):
        thread = threading.Thread(target=check_codes_thread)
        thread.start()
        threads.append(thread)

    # Ожидаем завершения всех потоков (хотя они бесконечные)
